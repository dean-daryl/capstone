...
def setup(
    ...
    jar: str = typer.Option(
        None,
        "--jar",
        help="Path to the backend JAR file (auto-detected if not provided)",
    ),
    skip_build: bool = typer.Option(
        False,
        "--skip-build",
        help="Skip building frontend/backend (use existing JAR)",
    ),
) -> None:
    ...
    if skip_build or jar:
        ...
        try:
            ...
            typer.echo(f"  Installed JAR to {dest}")
        except FileNotFoundError as e:
            typer.echo(f"\n  Build failed: {e}")
            typer.echo("  Falling back to JAR auto-detection...")
            try:
                locate_and_install_jar(jar_path=jar)
...
@app.command()
def build() -> None:
    """Build the frontend, bundle it into the backend, and create the JAR."""
    import shutil
    ...
    try:
        ...
        dest.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(str(jar_path), str(dest))
        typer.echo(f"\nInstalled JAR to {dest}")
        typer.echo("Run 'somatek start' to launch.")
